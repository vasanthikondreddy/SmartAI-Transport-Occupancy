...
const PortalDropdown = ({ anchorRef, open, options, value, onChange, onClose, accentColor = "#F59E0B" }: {
...
const CustomSelect = ({ label, value, onChange, options, placeholder, icon: Icon, accentColor = "#F59E0B", fontSize = "13px", padding = "11px 16px" }: {
    ...
    try {
      const res = await fetch(`${API_BASE}/predict/bus`, {
        ...
        body: JSON.stringify({
          ...
          source_city: sourceCity, destination_city: destCity,
          bus_type: busType, bus_capacity: Number(capacity), is_holiday: 0,
        }),
      });
      ...
      saveEntry({ type:"bus", sourceCity, destCity, travelDate, day, month, season, hourOfDep,
        vehicleClass:busType, capacity:Number(capacity),
        occupancyLevel:apiLevel, occupancyPercentage:pct,
        estimatedPassengers:data.estimated_passengers, suggestion:suggestions[level] });

      setHistFlash(true); setTimeout(() => setHistFlash(false), 2000);

      if (level === "high" || level === "very_high") {
        const notifMessages: Record<OccupancyLevel, { title:string; body:string }> = {
          very_high: { title:`🚨 Very High Occupancy — ${busType} Bus`,
            body:`${sourceCity} → ${destCity} on ${day} at ${formatHour(hourOfDep)} is extremely crowded (${pct}%). Book immediately or find an alternate!` },
          high:      { title:`⚠️ High Occupancy — ${busType} Bus`,
            body:`${sourceCity} → ${destCity} on ${day} at ${formatHour(hourOfDep)} is expected to be ${pct}% full. Book in advance!` },
          medium:    { title:"", body:"" }, low:{ title:"", body:"" },
        };
        sendNotification({ title:notifMessages[level].title, body:notifMessages[level].body,
          type:"bus", level:apiLevel, route:`${sourceCity} → ${destCity}` });
  ...
  const handleOpenCompare = () => {
    ...
    navigate("/compare/bus", {
      state: {
        ...
        baseRoute: {
          sourceCity, destCity, day, month, season, hourOfDep, busType, capacity,
        },
      },
    });
  };
      ...
      <header className="sticky top-0 z-40 glass-card border-b border-border/30 backdrop-blur-xl">
        <div className="container mx-auto px-6 py-4 flex items-center gap-4">
          ...
          <div className="flex items-center gap-3 flex-1">
            ...
            <div>
              <h1 className="font-display font-bold text-base text-foreground leading-none">Bus Occupancy Prediction</h1>
          ...
          <div style={{ display:"flex", alignItems:"center", gap:8 }}>
            ...
            <motion.button whileHover={{ scale:1.03 }} whileTap={{ scale:0.97 }}
              ...
              style={{ background: historyOpen ? "rgba(245,158,11,0.15)" : "rgba(245,158,11,0.08)",
                border:`1.5px solid ${histFlash || historyOpen ? "#F59E0B" : "rgba(245,158,11,0.3)"}`,
                color: histFlash || historyOpen ? "#B45309" : "hsl(var(--muted-foreground))",
                cursor:"pointer", transition:"all 0.2s" }}>
      ...
      <div className="transition-all duration-300" style={{ marginRight:(historyOpen || notifOpen) ? 288 : 0 }}>
        <div className="container mx-auto px-4 py-8 max-w-2xl space-y-6">
              ...
              <div key={s} style={{ display:"flex", alignItems:"center", flex:1 }}>
                <button onClick={() => i < step && setStep(i)}
                  ...
                  <div style={{ width:32, height:32, borderRadius:"50%", display:"flex", alignItems:"center",
                    ...
                    background: i<=step ? "#F59E0B" : "#3f3f46",
                    color: i<=step ? "#fff" : "#a1a1aa",
                    transform: i===step ? "scale(1.15)" : "scale(1)", transition:"background 0.3s, transform 0.3s" }}>
                    {i < step ? "✓" : i + 1}
                  </div>
                  <span style={{ fontSize:12, fontWeight:600,
                    color: i===step ? "#B45309" : "hsl(var(--muted-foreground))", transition:"color 0.3s" }}>{s}</span>
                </button>
                {i < steps.length - 1 && (
                  <div style={{ flex:1, height:1, margin:"0 12px",
                    background: i<step ? "#F59E0B" : "hsl(var(--border))", transition:"background 0.4s" }}/>
                )}
              </div>
            ))}
          </div>
          ...
          <div className="glass-card rounded-2xl overflow-hidden"
            style={{ border:"1.5px solid hsl(var(--border))", boxShadow:"0 8px 40px rgba(245,158,11,0.06)" }}>
            <div className="px-8 py-5 flex items-center gap-3"
              style={{ background:"linear-gradient(135deg,#1C1917,#44403C)" }}>
              <Zap className="w-5 h-5 opacity-90" style={{ color:"#F59E0B" }}/>
              <div>
                    ...
                    <AnimatePresence>
                      {sourceCity && (
                        <motion.div initial={{ opacity:0, scaleX:0 }} animate={{ opacity:1, scaleX:1 }}
                          ...
                          <div style={{ flex:1, height:1, background:"linear-gradient(to right, rgba(245,158,11,0.2), #F59E0B)" }}/>
                          <Bus style={{ width:18, height:18, color:"#F59E0B" }}/>
                          <div style={{ flex:1, height:1, background:"linear-gradient(to right, #F59E0B, rgba(245,158,11,0.2))" }}/>
                        </motion.div>
                      )}
                    </AnimatePresence>
                  ...
                  <motion.div key="s1" initial={{ opacity:0, x:30 }} animate={{ opacity:1, x:0 }} exit={{ opacity:0, x:-30 }} transition={{ duration:0.28 }} className="space-y-5">
                    <div className="space-y-1.5">
                      <label style={{ fontSize:"11px", fontWeight:600, letterSpacing:"0.08em", textTransform:"uppercase",
                        ...
                        <Calendar style={{ width:13, height:13, color:"#F59E0B" }}/> Travel Date
                      </label>
                      <input type="date" min={todayStr} value={travelDate}
                        ...
                        style={{ width:"100%", padding:"11px 16px", borderRadius:"12px", fontSize:"13px", fontWeight:500,
                          ...
                          border:`1.5px solid ${dateError ? "#ef4444" : travelDate && !dateError ? "#F59E0B" : "hsl(var(--border))"}`,
                          color: travelDate ? "hsl(var(--foreground))" : "hsl(var(--muted-foreground))",
                          boxShadow: dateError ? "0 0 0 3px rgba(239,68,68,0.12)" : travelDate && !dateError ? "0 0 0 3px rgba(245,158,11,0.15)" : "none",
                    ...
                    <div className="space-y-2">
                      <label style={{ fontSize:"11px", fontWeight:600, letterSpacing:"0.08em", textTransform:"uppercase",
                        color:"hsl(var(--muted-foreground))", display:"flex", alignItems:"center", gap:6 }}>
                        <Clock style={{ width:13, height:13, color:"#F59E0B" }}/> Hour of Departure
                      </label>
                      <div className="glass-card rounded-xl p-4" style={{ border:"1.5px solid hsl(var(--border))" }}>
                        <div style={{ textAlign:"center", marginBottom:10 }}>
                          <span className="font-display text-2xl font-bold" style={{ color:"#B45309" }}>{formatHour(hourOfDep)}</span>
                        </div>
                ...
                {step === 2 && (
                  <motion.div key="s2" initial={{ opacity:0, x:30 }} animate={{ opacity:1, x:0 }} exit={{ opacity:0, x:-30 }} transition={{ duration:0.28 }} className="space-y-5">
                    ...
                    <div className="space-y-1.5">
                      <label style={{ fontSize:"11px", fontWeight:600, letterSpacing:"0.08em", textTransform:"uppercase",
                        color:"hsl(var(--muted-foreground))", display:"flex", alignItems:"center", gap:6 }}>
                        <Users style={{ width:13, height:13, color:"#F59E0B" }}/> Bus Capacity
                      </label>
                    ...
                    <AnimatePresence>
                      {busType && (
                        <motion.div initial={{ opacity:0, y:6 }} animate={{ opacity:1, y:0 }}
                          style={{ padding:"10px 14px", borderRadius:10, background:"rgba(245,158,11,0.06)",
                            border:"1px solid rgba(245,158,11,0.25)", fontSize:12, color:"hsl(var(--muted-foreground))",
                            display:"flex", alignItems:"center", gap:8 }}>
                          <Bus style={{ width:14, height:14, color:"#F59E0B", flexShrink:0 }}/>
                          <span>
                    ...
                    <div style={{ borderRadius:12, padding:16, background:"rgba(245,158,11,0.06)", border:"1px dashed rgba(245,158,11,0.4)" }}>
                      <p style={{ fontSize:11, fontWeight:600, letterSpacing:"0.08em", textTransform:"uppercase", color:"#B45309", marginBottom:10 }}>Trip Summary</p>
                      {[
              ...
              <div style={{ display:"flex", gap:12, marginTop:28 }}>
                ...
                {step < 2 ? (
                  <Button onClick={() => setStep(s => s+1)} disabled={!stepValid[step]}
                    ...
                    style={{ height:44, background:"linear-gradient(135deg,#F59E0B,#B45309)", border:"none",
                      boxShadow: stepValid[step] ? "0 4px 14px rgba(245,158,11,0.35)" : "none" }}>
                    Continue
                  </Button>
                ) : (
                  <Button onClick={handlePredict} disabled={!stepValid[2] || loading}
                    className="flex-1 rounded-xl font-semibold text-white"
                    style={{ height:44, background:"linear-gradient(135deg,#F59E0B,#B45309)", border:"none",
                      boxShadow: stepValid[2] && !loading ? "0 4px 14px rgba(245,158,11,0.35)" : "none" }}>
              ...
              <motion.div initial={{ opacity:0 }} animate={{ opacity:1 }} exit={{ opacity:0 }}
                ...
                <div style={{ position:"relative", width:56, height:56 }}>
                  <motion.div animate={{ rotate:360 }} transition={{ repeat:Infinity, duration:1.4, ease:"linear" }}
                    style={{ position:"absolute", inset:0, borderRadius:"50%", border:"3px solid rgba(245,158,11,0.2)", borderTopColor:"#F59E0B" }}/>
                  <Bus style={{ position:"absolute", top:"50%", left:"50%", transform:"translate(-50%,-50%)", width:22, height:22, color:"#F59E0B" }}/>
                </div>
              ...
              <motion.div initial={{ opacity:0 }} animate={{ opacity:1 }} exit={{ opacity:0 }} className="space-y-5">
                  ...
                  <motion.div initial={{ opacity:0, y:-10 }} animate={{ opacity:1, y:0 }}
                    ...
                    <p style={{ fontSize:12, color:"hsl(var(--foreground))", fontWeight:500 }}>
                      {result.level==="very_high"
                        ? "🚨 Alert sent! Very high occupancy detected — check notifications for details."
                ...
                <motion.div initial={{ opacity:0, y:16 }} animate={{ opacity:1, y:0 }} transition={{ delay:0.1 }}>
                  <Button
                    ...
                    <span style={{ marginLeft:8, fontSize:10, padding:"2px 8px", borderRadius:999,
                      background:"rgba(245,158,11,0.2)", color:"#B45309", fontWeight:700 }}>
                      OPEN FULL PAGE
                    </span>
                  </Button>
                </motion.div>
                ...
                <motion.div initial={{ opacity:0, y:30 }} animate={{ opacity:1, y:0 }} transition={{ delay:0.2 }}
                  ...
                  <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:24 }}>
                    <BarChart2 style={{ width:18, height:18, color:"#F59E0B" }}/>
                  ...
                  <div className="space-y-8">
                    ...
                    <AnimatedBarChart data={result.weeklyData} color="#F59E0B" labels={["Mon","Tue","Wed","Thu","Fri","Sat","Sun"]} title="Weekly Pattern"/>
                  </div>
                </motion.div>
                ...
                <motion.div initial={{ opacity:0, y:30 }} animate={{ opacity:1, y:0 }} transition={{ delay:0.35 }}
                  ...
                  <h3 className="font-display font-bold text-foreground" style={{ display:"flex", alignItems:"center", gap:8, marginBottom:16 }}>
                    <Zap style={{ width:16, height:16, color:"#F59E0B" }}/> Smart Travel Tips
                  </h3>
                  <ul className="space-y-3">
                    {(result.level==="high"||result.level==="very_high" ? [
                      ...
                      { dot:"#F59E0B", text:`${season} season can affect occupancy — plan ahead for holidays.` },
                      { dot:"#F59E0B", text:`${busType} buses typically see higher demand on ${day}s.` },
                    ]).map(({ dot, text }, i) => (
