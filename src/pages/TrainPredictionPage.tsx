...
const PortalDropdown = ({ anchorRef, open, options, value, onChange, onClose }: {
  ...
  return createPortal(
    <AnimatePresence>
      {open && (
        <motion.div
          initial={{ opacity: 0, y: openUp ? 8 : -8, scaleY: 0.94 }}
          animate={{ opacity: 1, y: 0, scaleY: 1 }}
          exit={{ opacity: 0, y: openUp ? 8 : -8, scaleY: 0.94 }}
...
const TrainPredictionPage = () => {
  ...
  // Auto-derive day, month, season from date
  useEffect(() => {
    ...
    try {
      const res = await fetch(`${API_BASE}/predict/train`, {
        ...
        body: JSON.stringify({
          ...
          source_station:       sourceCity,
          destination_station:  destCity,
          travel_class:         vehicleClass,
          train_type:           trainType,
          railway_zone:         railwayZone,
          seat_capacity:        Number(capacity),
          route_distance_km:    Number(routeDistance),
          is_holiday:           0,
            ...
            <div key={s} style={{ display: "flex", alignItems: "center", flex: 1 }}>
              <button onClick={() => i < step && setStep(i)}
                ...
                <span style={{ fontSize: 12, fontWeight: 600,
                  color: i === step ? "hsl(var(--primary))" : "hsl(var(--muted-foreground))", transition: "color 0.3s" }}>{s}</span>
              </button>
              {i < steps.length-1 && (
                <div style={{ flex: 1, height: 1, margin: "0 12px",
                  background: i < step ? "hsl(var(--primary))" : "hsl(var(--border))", transition: "background 0.4s" }} />
              )}
            </div>
          ))}
        </div>
            ...
            <AnimatePresence mode="wait">
                  ...
                  <AnimatePresence>
                    {sourceCity && (
                      <motion.div initial={{ opacity:0, scaleX:0 }} animate={{ opacity:1, scaleX:1 }}
                        ...
                        <Train style={{ width:18, height:18, color:"hsl(var(--primary))" }}/>
              ...
              {step === 2 && (
                <motion.div key="s2" initial={{ opacity:0, x:30 }} animate={{ opacity:1, x:0 }} exit={{ opacity:0, x:-30 }} transition={{ duration:0.28 }}
                  ...
                  <CustomSelect label="Train Class" value={vehicleClass} onChange={setVehicleClass}
                    options={trainClasses} placeholder="Select train class" />

                  <CustomSelect label="Train Type" value={trainType} onChange={setTrainType}
                    options={trainTypes} placeholder="Select train type" />

                  <CustomSelect label="Railway Zone" value={railwayZone} onChange={setRailwayZone}
                    options={railwayZones} placeholder="Select zone e.g. SCR" />

                  <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}>
                    <div className="space-y-1.5">
                      <label style={{ fontSize:"11px", fontWeight:600, letterSpacing:"0.08em", textTransform:"uppercase",
                        color:"hsl(var(--muted-foreground))", display:"flex", alignItems:"center", gap:6 }}>
                        <Users style={{ width:13, height:13, color:"hsl(var(--primary))" }}/> Coach Capacity
                      </label>
                      <Input type="number" placeholder="e.g. 72" min={18} max={120} value={capacity}
                        onChange={e => setCapacity(e.target.value)}
                        className="rounded-xl" style={{ border:"1.5px solid hsl(var(--border))",
                          background:"hsl(var(--card))", padding:"11px 16px", fontSize:13 }}/>
                      <p style={{ fontSize:10, color:"hsl(var(--muted-foreground))" }}>SL/3A=72, 2A=46, 1A=18</p>
                    </div>
                    <div className="space-y-1.5">
                      <label style={{ fontSize:"11px", fontWeight:600, letterSpacing:"0.08em", textTransform:"uppercase",
                        color:"hsl(var(--muted-foreground))", display:"flex", alignItems:"center", gap:6 }}>
                        <MapPin style={{ width:13, height:13, color:"hsl(var(--primary))" }}/> Distance (KM)
                      </label>
                  ...
                  <AnimatePresence>
                    {vehicleClass && (
                      <motion.div initial={{ opacity:0, y:6 }} animate={{ opacity:1, y:0 }}
                        ...
                        <Train style={{ width:14, height:14, color:"hsl(var(--primary))", flexShrink:0 }}/>
                        <span>
          ...
          {loading && (
            <motion.div initial={{ opacity:0 }} animate={{ opacity:1 }} exit={{ opacity:0 }}
              ...
              <div style={{ position:"relative", width:56, height:56 }}>
                ...
                <Train style={{ position:"absolute", top:"50%", left:"50%", transform:"translate(-50%,-50%)", width:22, height:22, color:"hsl(var(--primary))" }}/>
              </div>
              ...
              <motion.div initial={{ opacity:0, y:30 }} animate={{ opacity:1, y:0 }} transition={{ delay:0.35 }}
                ...
                <ul className="space-y-3">
                  {(result.level==="high"||result.level==="very_high" ? [
                    ...
                    { dot:"hsl(var(--primary))", text:`${vehicleClass} coaches typically see higher demand on ${day}s.` },
                  ]).map(({ dot, text }, i) => (
